...
export async function POST(request) {
  ...
  try {
    ...
    const { id_paciente, id_medico, glucosa_mg_dl, presion_sistolica, presion_diastolica, peso_kg, imc, sintomas } = data;

    await connection.beginTransaction();

    const [result] = await connection.query(
      `INSERT INTO MEDICION_CLINICA 
       (id_paciente, id_medico, glucosa_mg_dl, presion_sistolica, presion_diastolica, peso_kg, imc) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id_paciente, id_medico, glucosa_mg_dl, presion_sistolica, presion_diastolica, peso_kg, imc]
    );
    ...
    const payloadDespues = {
      id_medicion, id_paciente, id_medico, glucosa_mg_dl, presion_sistolica, presion_diastolica, peso_kg, imc, sintomas
    };
